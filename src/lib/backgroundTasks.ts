...
const ANTI_BIAS_RULE = `ŞIK DAĞILIMI, UZUNLUK VE İNANDIRICILIK — ÇOK ÖNEMLİ, kesinlikle uy: Bir sorudaki "correct_index" değerinin hep aynı konumda olması, doğru şıkkın diğerlerinden uzun/detaylı yazılması (VEYA diğerlerinin doğru şıkka yaklaşmaya çalışıp kendi aralarında daha kısa kümelenmesi) ya da yanlış şıkların mantıksal olarak imkansız/saçma olması, kullanıcının içeriği hiç okumadan veya konuyu hiç bilmeden salt kalıba bakarak doğru cevabı bulmasına yol açar. Bunu önlemek için:
  ...
  (5) UZUNLUK SIRASI ÇEŞİTLİLİĞİ: Tek bir soru mükemmel dengeli olsa bile, bir ünitedeki/sınavdaki TÜM sorularda doğru şık hep "en uzun ya da en kısaya en yakın" şık olursa (küçük farklarla bile olsa), okuyucu 10 soru boyunca bunu fark edip örüntüyü öğrenebilir. Bunu önlemek için: 10 sorunun İÇİNDE doğru şıkkın o sorudaki uzunluk sırası (o soruda en kısa şık mı, en uzun şık mı, ortancalardan biri mi) bilinçli olarak DEĞİŞSİN — doğru şık kimi sorularda en kısa/en sade şık, kimi sorularda en uzun/en detaylı şık, kimi sorularda ortada olsun; art arda aynı uzunluk sırasında 3'ten fazla soru olmasın.`;
